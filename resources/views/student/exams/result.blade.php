...
@if(!app(\App\Services\ResultPublicationService::class)->legacyAttemptVisible($attempt->exam))<div class="notice">تم استلام المحاولة. تظهر النتائج المعتمدة بعد نشر الإدارة في <a href="{{ route('student.results') }}">قسم النتائج</a>.</div>@elseif($attempt->status === 'pending_review')<div class="notice">أُرسلت المحاولة وتنتظر مراجعة الأسئلة غير الموضوعية.</div>@else<section class="metrics-grid"><article class="metric"><span>الدرجة</span><strong>{{ $score }} / {{ $maximum }}</strong></article><article class="metric"><span>النسبة</span><strong>{{ $percentage }}%</strong></article></section>@endif
