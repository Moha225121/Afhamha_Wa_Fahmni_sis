...
<span class="teacher-class-card__header"><small>الفصل {{ $classroom->section ?: 'الدراسي' }}</small><strong>{{ $classroom->name }}</strong><span class="teacher-class-card__header-subjects">@forelse($classroom->assignment_labels as $label)<span>{{ $label }}</span>@empty<span>لا توجد مادة</span>@endforelse</span></span>
