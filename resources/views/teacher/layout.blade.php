...
<div class="page"><header class="topbar"><form class="global-search"><input aria-label="بحث" placeholder="ابحث في المنصة..."></form><div class="account"><a href="{{ route('teacher.profile.edit') }}" class="profile-link">@if(auth()->user()->avatar_path)<img class="teacher-header-avatar" src="{{ route('teacher.profile.avatar') }}" alt="">@else<span class="teacher-header-avatar teacher-header-avatar--fallback">{{ mb_substr(auth()->user()->name, 0, 1) }}</span>@endif{{ auth()->user()->name }}</a><form method="post" action="{{ route('logout') }}">@csrf<button>خروج</button></form></div></header>
