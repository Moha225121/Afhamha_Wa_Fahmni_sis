...
class ResultPublicationService
{
    ...
    public function visibleResults(?Student $student): Collection
    {
        ...
        $published = $this->publications($student)->flatMap(fn ($p) => collect($p->result['subjects'])->map(fn ($s) => (object) ['score' => $s['score'], 'total_score' => $s['maximum_score'], 'title' => $p->period, 'subject' => $s['subject'], 'published_at' => $p->published_at]));
        $manual = DB::table('grades as g')->join('exams as e', 'e.id', '=', 'g.exam_id')->join('subjects as s', 's.id', '=', 'e.subject_id')->where('g.student_id', $student->id)->where('e.legacy_results_published', true)->whereNull('e.exam_period_id')->where('e.status', 'published')->where('g.published_at', '<=', now())->select('e.id as exam_id', 'g.score', 'e.total_score', 'e.title', 's.name as subject', 'g.published_at')->get();
        $automatic = DB::table('exam_attempts as a')->join('exams as e', 'e.id', '=', 'a.exam_id')->join('subjects as s', 's.id', '=', 'e.subject_id')->where('a.student_id', $student->id)->where('a.status', 'submitted')->whereNotNull('a.percentage')->where('e.legacy_results_published', true)->whereNull('e.exam_period_id')->where('e.status', 'published')->whereNotIn('e.id', $manual->pluck('exam_id'))->select('e.id as exam_id', 'a.score', 'a.maximum_score as total_score', 'e.title', 's.name as subject', 'a.submitted_at as published_at')->orderByDesc('a.id')->get()->unique('exam_id');
